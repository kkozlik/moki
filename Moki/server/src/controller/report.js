...
const { getQueries } = require('../utils/metrics');
...
class ReportController {
  static getReport(req, res, next) {
    async function search() {
      ...
      //INCIDENT COUNT
      const incidentCount = datehistogram_query.getTemplate(timebucket, getQueries(filters, types, timestamp_gte, timestamp_lte, userFilter, "*"), supress);

      //INCIDENT COUNT -1 day
      const incidentCountDayAgo = datehistogram_query.getTemplate(timebucket, getQueries(filters, types, (timestamp_gte - 60 * 60 * 24 * 1000), (timestamp_lte - 60 * 60 * 24 * 1000), userFilter, "*"), supress);

      //DESTINATIONS CAs STATISTICS
      const statsCA = multiple_query.getTemplate("attrs.dst_ca_id", "attrs.durationMin", "CallEnd", "CallAttempts", "SumFailureSuccess", "failure", getQueries(filters, types, timestamp_gte, timestamp_lte, userFilter, "*"), supress);
