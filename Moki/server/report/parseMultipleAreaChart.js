/*
Parsing for multiple area chart
data format: 
    name: name,
    values: time, value

*/
function parseMultipleAreaChart(responseName, response, responseName2, response2) {
    if (response.aggregations && response2.aggregations && response.aggregations.agg && response2.aggregations.agg) {
        ...
        for (var j = 0; j < areachartDataParse2.length; j++) {
            if (areachartDataParse2[j].agg &&
                areachartDataParse2[j].agg.value && areachartDataParse2[j].agg.value !== null) {
        ...
        for (j = 0; j < areachartDataParse.length; j++) {
            if (areachartDataParse[j].agg &&
                areachartDataParse[j].agg.value && areachartDataParse[j].agg.value !== null) {
                        console.log(areachartDataParse[j].agg.value);
